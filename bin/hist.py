...
zsh_config_folder = "/".join([config_folder, "zsh"])
default_db_location = "/".join([zsh_config_folder, ".zsh_hist_backup.db"])
...
def restore(history_path=None, db_name=None, max_lines=None):
    ...
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM " + default_table_name + " ORDER BY timestamp desc")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--maxlines",
        dest="maxlines",
        help="maximum size of history file",
        default=-1,
    )
